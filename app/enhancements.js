(() => {
  ...
  async function instagramQuote(event) {
    ...
    try { legacyCopied = document.execCommand('copy'); } catch {}
    area.remove();
    const copyPromise = (navigator.clipboard?.writeText && window.isSecureContext)
      ? navigator.clipboard.writeText(text).then(()=>true).catch(()=>legacyCopied)
      : Promise.resolve(legacyCopied);
    const link = document.createElement('a');
    link.href = `https://ig.me/m/${encodeURIComponent(handle)}`;
    link.target = '_blank';
    ...
    link.click();
    link.remove();
    copyPromise.then(copied => showToast(copied ? 'Cotización copiada. Pégala en Instagram.' : 'Instagram abierto. Mantén pulsado y pega la cotización.'));
  }
