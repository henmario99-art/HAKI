(() => {
  const FALLBACK = window.HAKI_GYMRAT_DEFAULTS || {
    habilitado:true, botonTexto:'GYMRAT TEST', titulo:'GYMRAT TEST', introduccion:'Responde y descubre tu perfil.',
    preguntas:[
  ...
  function settings(){
    if (typeof window.hakiGymratSettings === 'function') return window.hakiGymratSettings(window.HAKI_CONFIG || {});
    return {...FALLBACK,...(window.HAKI_CONFIG?.gymrat || {})};
  }
  ...
  function ensureDialog(){
    ...
    dialog.id='gymratDialog'; dialog.className='gymrat-dialog';
  ...
  function productCard(product){
    ...
    return `<a class="gymrat-product" href="#producto/${encodeURIComponent(product.codigo)}">
      ...
      <div class="gymrat-product-meta"><div><span class="gymrat-product-code">${esc(product.codigo)}</span><span class="gymrat-product-name">${esc(product.nombre)}</span></div><span class="gymrat-product-price">${esc(money(product.precio))}</span></div>
    </a>`;
  }
  ...
  function renderQuestion(dialog,state){
    const cfg=state.cfg;
    const question=cfg.preguntas[state.index];
    ...
    stage.innerHTML=`<div class="gymrat-question-view">
      ...
      <span class="gymrat-question-number">Pregunta ${state.index+1}</span>
      <h3 class="gymrat-question-title">${esc(question.texto)}</h3>
      <div class="gymrat-options">${(question.opciones||[]).map(option=>`<button class="gymrat-option" type="button" data-value="${esc(option)}">${esc(option)}</button>`).join('')}</div>
    </div>`;
    stage.querySelectorAll('.gymrat-option').forEach(button=>{
      button.addEventListener('click',()=>{
        ...
        const advance=()=>{
          state.index++;
          state.locked=false;
          if(state.index<cfg.preguntas.length)renderQuestion(dialog,state); else reveal(dialog,state);
        };
  ...
  function showResult(dialog,state){
    const cfg=state.cfg;
    const key=scoresFor(state.answers,cfg);
    const profile=cfg.perfiles?.[key]||cfg.perfiles?.dark||{};
    const products=recommendations(profile,key,cfg);
    const stage=dialog.querySelector('.gymrat-stage');
    const result=dialog.querySelector('.gymrat-result');
    stage.hidden=true;
    result.innerHTML=`<span class="gymrat-result-badge">TU RESULTADO</span>
      <h2 class="gymrat-result-title">${esc(profile.nombre||'GYMRAT')}</h2>
      <p class="gymrat-result-copy">${esc(profile.texto||'')}</p>
      <div class="gymrat-recs-head"><div><h3>SELECCIÓN PARA TI</h3><p>Filtrada por los colores catalogados de cada prenda.</p></div></div>
      ${products.length?`<div class="gymrat-products">${products.map(productCard).join('')}</div>`:`<p class="gymrat-empty-recs">Todavía no hay prendas catalogadas con los colores de este perfil. Puedes asignar colores a cada producto desde el panel de administración.</p>`}
      <div class="gymrat-result-actions"><button class="gymrat-restart" type="button">REPETIR TEST</button><a class="gymrat-shop" href="#catalogo">VER TODAS LAS PRENDAS</a></div>`;
    result.classList.add('active');
    result.querySelector('.gymrat-restart')?.addEventListener('click',()=>startQuiz(dialog,cfg));
  ...
  function startQuiz(dialog,cfg=settings()){
    const questions=(cfg.preguntas||[]).slice(0,4);
    cfg={...cfg,preguntas:questions};
    dialog.innerHTML=shellMarkup(cfg);
    dialog.querySelector('.gymrat-close')?.addEventListener('click',()=>dialog.close());
    const state={cfg,index:0,answers:{},locked:false};
  ...
  function openQuiz(event){
    ...
    const cfg=settings();
    if(cfg.habilitado===false||cfg.habilitado==='false')return;
    const dialog=ensureDialog();
    startQuiz(dialog,cfg);
    dialog.showModal();
  }
  ...
  function setupButton(){
    ...
    if(!button)return;
    button.textContent=cfg.botonTexto||'GYMRAT TEST';
    button.href='#gymrat-test';
    button.hidden=cfg.habilitado===false||cfg.habilitado==='false';
    if(button.dataset.gymratV2Ready!=='1'){
      button.dataset.gymratV2Ready='1';
      button.addEventListener('click',openQuiz);
    }
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',setupButton,{once:true}); else setupButton();
