(() => {
  ...
  const groups={
    'Apariencia':[['tema','Modo del catálogo','claro|oscuro'],['fuenteTitulos','Fuente de títulos','Horizon|Poppins'],['fuenteTexto','Fuente de subtítulos y textos','Poppins|Horizon'],['horizonUrl','Archivo web de Horizon (URL WOFF2, WOFF o TTF)']],
    'Anuncios y portada':[['anuncio2','Segundo anuncio'],['anuncio3','Tercer anuncio'],['botonPortada','Texto del primer botón'],['enlacePortada','Destino del primer botón'],['botonPortada2','Texto del segundo botón'],['enlacePortada2','Destino del segundo botón'],['buscarTexto','Mensaje del buscador']],
  ...
  Object.entries(groups).forEach(([title,fields])=>{
    ...
    fields.forEach(([key,text,type])=>{
      ...
      if(type?.includes('|'))type.split('|').forEach(value=>{const o=document.createElement('option');o.value=value;o.textContent=value==='true'?'Sí, por prenda':value==='false'?'No, por pedido':value;input.append(o);});
  ...
  const note=document.createElement('p');note.textContent='Horizon necesita su archivo para uso web. Hasta cargarlo se muestra Poppins. Los anuncios cambian cada 5 segundos. Los destinos de botones pueden ser #catalogo, #novedades o un enlace.';card.append(note);
