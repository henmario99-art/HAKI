...
function switchTab(name){if(name!=='sales')closeArchivedSales();$('.tab').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.tab===name));$('#salesView').hidden=name!=='sales';$('#inventoryView').hidden=name!=='inventory';if(name==='inventory')renderInventory()}
...
function closeArchivedSales(){const view=$('#archivedSalesView');if(view)view.hidden=true;const days=$('#days');if(days)days.hidden=false;const metrics=$('.metrics');if(metrics)metrics.hidden=false;const button=$('#archivedSales');if(button)button.textContent='Archivadas'}
...
function renderArchivedSales(){const wrap=$('#archivedSalesList');if(!wrap)return;wrap.replaceChildren();if(!state.archived.length){const empty=document.createElement('div');empty.className='empty-day';empty.textContent='No hay ventas archivadas.';wrap.append(empty);return}state.archived.forEach(sale=>{const card=document.createElement('article');card.className='haki-sale-mini';card.style.marginBottom='12px';const items=(sale.items||[]).map(i=>`${i.codigo} ${i.talla}${Number(i.cantidad)>1?` ×${i.cantidad}`:''}`).join(' · ')||'Pedido';card.innerHTML=`<div><strong>${escapeHtml(sale.cliente||'Cliente')}</strong><small>${escapeHtml(items)}</small></div><div class="amount">${money(sale.total)}</div><div class="meta"><span class="pill cancelled">Archivada</span><span class="pill">${escapeHtml(formatAuditTime(sale.archivedAt)||'')}</span></div>`;const actions=document.createElement('div');actions.style.display='flex';actions.style.flexWrap='wrap';actions.style.gap='8px';actions.style.marginTop='12px';const restore=document.createElement('button');restore.type='button';restore.className='button primary';restore.textContent='Restaurar venta';restore.addEventListener('click',async()=>{if(!confirm(`¿Restaurar la venta de ${sale.cliente||'este cliente'}? El sistema volverá a reservar el inventario.`))return;restore.disabled=true;try{await api('sales?mode=restore',{method:'POST',body:JSON.stringify({id:sale.id})});await loadArchivedSales();await loadWeek();toast('Venta restaurada y stock reservado nuevamente.')}catch(err){toast(err.message,true)}finally{restore.disabled=false}});const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent='Historial';summary.style.cursor='pointer';const history=document.createElement('div');history.style.padding='8px 0';details.append(summary,history);details.addEventListener('toggle',()=>{if(details.open)loadSaleHistory(sale.id,history)});actions.append(restore,details);card.append(actions);wrap.append(card)})}
...
async function openArchivedSales(){hideEditor();const view=$('#archivedSalesView');if(!view)return;view.hidden=false;$('#days').hidden=true;$('.metrics').hidden=true;$('#archivedSales').textContent='Actualizando…';try{await loadArchivedSales();$('#archivedSales').textContent='Archivadas'}catch(err){toast(err.message,true);closeArchivedSales()}}
...
$('#newSale').addEventListener('click',()=>{closeArchivedSales();const today=isoDate(new Date());openNewSale(mondayOf(today)===state.weekStart?today:state.weekStart)});
$('#archivedSales').addEventListener('click',openArchivedSales);
$('#backFromArchived').addEventListener('click',closeArchivedSales);
