...
function signalStockChange(){try{localStorage.setItem('haki_stock_version',String(Date.now()))}catch{}}
...
async function saveSale(){const button=$('#saveSale');button.disabled=true;const wasEditing=!!state.editing;try{const sale=formSale();if(!sale.fecha)throw new Error('Selecciona el día que retiró el cliente.');if(!sale.cliente.trim())throw new Error('Escribe el nombre del cliente.');const options=wasEditing?{method:'PUT',body:JSON.stringify({sale,previousWeekStart:state.previousWeekStart})}:{method:'POST',body:JSON.stringify({sale})};const data=await api('sales',options);hideEditor();state.weekStart=data.weekStart||state.weekStart;signalStockChange();await loadWeek();toast(wasEditing?'Venta actualizada':'Venta guardada')}catch(err){toast(err.message,true)}finally{button.disabled=false}}
async function deleteSale(){if(!state.editing||!confirm(`¿Eliminar la venta de ${state.editing.cliente||'este cliente'}? El stock reservado se devolverá.`))return;const button=$('#deleteSale');button.disabled=true;try{await api('sales',{method:'DELETE',body:JSON.stringify({id:state.editing.id,weekStart:state.previousWeekStart})});hideEditor();signalStockChange();await loadWeek();toast('Venta eliminada')}catch(err){toast(err.message,true)}finally{button.disabled=false}}
